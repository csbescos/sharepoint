...
# We also check the group document library in SharePoint Online to see whether it exists or has been used in the last 30 days.
...
#Variable for the sharepoint admin site.  Example is https://tenantname-admin.sharepoint.com
$SpoAdminSite = 

##Connect to the three different modules.
Connect-ExchangeOnline
Connect-SPOService -Url $SpoAdminSite
...
$WarningDate = (Get-Date).AddDays(-90); $WarningEmailDate = (Get-Date).AddDays(-90); $Today = (Get-Date); $Date = $Today.ToShortDateString()
...
# Generate the number of warnings to decide how obsolete the group might be...   
  If ($AuditRecs -eq $Null) {
       $SPOActivity = "No SPO activity detected in the last 30 days"
       $NumberWarnings++ }
